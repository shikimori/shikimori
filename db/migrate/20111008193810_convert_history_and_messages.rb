class ConvertHistoryAndMessages < ActiveRecord::Migration
  def self.up
    ...
    ap AnimeHistory.connection().update_sql("update anime_histories set action='%s' where action='anime_released'" % AnimeHistoryAction::Released)
    ...
    ap Message.connection().update_sql("update messages set message_type='%s' where message_type='anime_released'" % AnimeHistoryAction::Released)
  end
  ...
  def self.down
    ...
    ap AnimeHistory.connection().update_sql("update anime_histories set action='anime_released' where action='%s'" % AnimeHistoryAction::Released)
    ...
    ap Message.connection().update_sql("update messages set message_type='anime_released' where message_type='%s'" % AnimeHistoryAction::Released)
  end
end
