class SetupGenreV2Positions < ActiveRecord::Migration[7.0]
  def change
    ...
    GenreV2.where(name: 'Shounen').update_all position: 11
    GenreV2.where(name: 'Shoujo').update_all position: 12
    GenreV2.where(name: 'Seinen').update_all position: 13
    GenreV2.where(name: 'Josei').update_all position: 14
