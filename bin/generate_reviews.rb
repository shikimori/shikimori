...
[Anime, Manga].each do |klass|
  ...
  Comment.
    ...
    order(id: :desc).
    limit(1000).where(commentable_id: [Anime.find(31240).decorate.main_topic_view.id, Manga.find(2).decorate.main_topic_view.id, Ranobe.find(9115).decorate.main_topic_view.id]).
