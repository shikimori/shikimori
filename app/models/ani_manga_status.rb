class AniMangaStatus
  ...
  def self.query_for(status, klass=Anime)
    ...
    case status
      when 'ongoing'
        "((aired_at is not null and aired_at < '%s 00:00:00' and status in ('%s', '%s')) " % [Date.today-AniManga::OngoingToReleasedDays.days, AniMangaStatus::Anons, AniMangaStatus::Upcoming] +
        " or (aired_at is not null and status in ('%s','%s'))) " % [AniMangaStatus::Ongoing, AniMangaStatus::Publishing] +
        "and not (aired_at is not null and released_at is not null and aired_at < date_add(now(), interval -#{AniManga::OngoingToReleasedDays} day) && released_at < date_add(now(), interval -7 day))" +
        "and not (status in ('%s', '%s') and aired_at = '%s-01-01 00:00:00') " % [AniMangaStatus::Anons, AniMangaStatus::Upcoming, Date.today.year] +
