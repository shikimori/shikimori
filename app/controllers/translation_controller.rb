  ...
  def index
    ...
    @goals << ['Top 50 TV',
               Anime.where(:id =>
                     ...
                     order(:ranked)]
    @goals << ['Top 20 OVA',
               Anime.where(id: Anime.where("kind != 'TV' and kind != 'Movie'").where.not(ranked: 0).order(:ranked).limit(40).pluck(:id)).
                     where.not(id: Anime::EXCLUDED_ONGOINGS).
                     where.not(id: @translate_ignore).
                     ...
                     limit(20)]
    @goals << ['Top 30 Movies',
               Anime.where(id: Anime.where(kind: 'Movie').where.not(ranked: 0).order(:ranked).limit(100).pluck(:id)).
                     where.not(id: Anime::EXCLUDED_ONGOINGS).
                     where.not(id: @translate_ignore).
                     where.not(id: goals_ids).
                     where(censored: false).
    ...
    @goals << ['Лето 2014',
               Anime.where(AniMangaSeason.query_for('summer_2014')).
                     where.not(id: goals_ids).
                     where('score > 0 or ranked > 0').
                     #where('duration is null or duration = 0 or duration > 20').
                     where.not(id: @translate_ignore).
    ...
    @goals << ['Подборка фильмов',
               Anime.where(id: [5310,7222,1034,1824,1379,2594,6637,1089,9760,464,6372,570,974,8247,885,3089,597,31,3087,441,867,713,743,462,869,866,868,867,536,2889,6162,463,1192,4835,493,1686,465,2472,459,522,1815,442,460,461,155,4970,8246,4437,405,4246,2811,1829,936,9000,8115,21647]).
                     where.not(id: goals_ids).
                     order(:ranked)]
    @goals << ['Подборка 9',
