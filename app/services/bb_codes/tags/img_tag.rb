    ...
    if is_no_zoom
      <<~HTML.squish
        <span class='b-image no-zoom#{" #{ERB::Util.h css_class}" if css_class.present?}'
          data-attrs='#{ERB::Util.h attrs.to_json}'>#{image_html}</span>
      HTML
    else
      <<~HTML.squish
        <a href='#{ERB::Util.h(link_url || image_url)}'
          data-href='#{ERB::Util.h(camo_link_url || fixed_image_url)}'
          rel='#{text_hash}'
          class='b-image unprocessed#{" #{ERB::Util.h css_class}" if css_class.present?}'
          data-attrs='#{ERB::Util.h attrs.to_json}'>#{image_html}</a>
      HTML
    end
  end

  def html_for_image fixed_image_url, width, height
    sizes_html = ''
    sizes_html += " width='#{width}'" if width.positive?
    sizes_html += " height='#{height.to_i}'" if height.positive?

    <<-HTML.squish.strip
      <img src='#{ERB::Util.h fixed_image_url}'
