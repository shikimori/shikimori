class FindAnimeImporter
  ...
  def import ids: [], pages: [], full: true
    fetch_pages(pages).each {|(id, anime, videos)| import_videos anime, videos, full } if pages.any?
    fetch_ids(ids).each {|(id, anime, videos)| import_videos anime, videos, full } if ids.any?
