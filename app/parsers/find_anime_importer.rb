...
private
  ...
  def video_uniq_url?(video)
    !(
      video.errors.one? &&
      (
        video.errors[:url].include?(I18n.t 'activerecord.errors.messages.taken') ||
        video.errors[:url].include?(I18n.t 'activerecord.errors.models.videos.attributes.url.taken')
      )
    )
  end
