...
class WorldArtParser < SiteParserWithCache
  ...
  ContentRegexp = /<td width=1><\/td><td valign=top><font size=3><b>(.*?) \[<\/font><a href='(?:http:\/\/www.world-art.ru\/animation\/)?list.php\?public_year=\d+'><font size=3 color=#990000>(\d+)<\/font><\/a><font size=3>\]<\/b><\/font>(.*)<font size=2><br><br><b>/
  ...
  def fetch_animes(reload=false, threads_num=100)
    ...
    pbar = ProgressBar.new("fetching animes...", data.size) if Rails.env == 'development'
    if threads_num > 1
      data.parallel(:threads => threads_num, :timeout => 45) do |anime_id|
  ...
  def merge_with_database
    ...
    found = 0
    pbar = ProgressBar.new("merging with WorldArt animes", @cache[:animes].size)
