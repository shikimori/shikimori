...
$('.b-comment .b-shiki_editor').live('ajax:before', (e) ->
  $this = $(@)
...
$('.b-comment .item-quote').live 'click', ->
...
# edit message
$('.b-comment .main-controls .item-edit').live 'ajax:success', (e, data, status, xhr) ->
  $replacement = $(data)
  $node = $block(@)
  ...
  new ShikiEditor($replacement).edit_comment($node)
  #$replacement.shiki_editor()#.addClass($node.attr('class').match(/comment-\d+/)[0]).show()
  #$replacement.find('textarea').focus()

# deletion
$('.main-controls .item-delete').live 'click', ->
  $('.main-controls', $block(@)).hide() $('.delete-controls', $block(@)).show()
