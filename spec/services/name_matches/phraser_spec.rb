describe NameMatches::Phraser do
  ...
  describe '#multiply' do
    it { expect(phraser.multiply ['zzz 2nd season'], /2nd season/, 's2')
      .to eq ['zzz 2nd season', 'zzz s2'] }
    it { expect(phraser.multiply ['mahou shoujo madoka magica'], 'magica', 'magika')
      .to eq ['mahou shoujo madoka magica', 'mahou shoujo madoka magika'] }
  end

  describe '#replace' do
    let(:phrases) { ['test', 'teest', 'test2'] }
    it { expect(phraser.replace phrases, /te+/, 'te').to eq ['test', 'test2'] }
  end
