describe EmailNotifier do
  ...
  describe '#private_message' do
    ...
    before { allow(Sendgrid).to receive(:delay_for).and_return mailer_double }
    ...
    context 'target user allowed private emails' do
      ...
      context 'target user is online' do
        ...
        it { expect(Sendgrid).to have_received(:delay_for).with(EmailNotifier::ONLINE_USER_MESSAGE_DELAY) }
      end

      context 'taget user is not online' do
        let(:last_online_at) { Time.zone.now - User::LAST_ONLINE_CACHE_INTERVAL - 1.second }
        it { expect(Sendgrid).to have_received(:delay_for).with(EmailNotifier::OFFLINE_USER_MESSAGE_DELAY) }
      end
    end

    context 'target user did not allow private emails' do
      let(:notifications) { 0 }
      it { expect(Sendgrid).to_not have_received(:delay_for) }
    end
  end
end
