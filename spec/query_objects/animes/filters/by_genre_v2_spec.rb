describe Animes::Filters::ByGenreV2 do
  subject { described_class.call Anime.order(:id), terms }

  let(:shounen) { create :genre_v2 }
  let(:shoujo) { create :genre_v2 }

  let!(:anime_1) { create :anime, genre_v2_ids: [shounen.id, shoujo.id] }
  let!(:anime_2) { create :anime, genre_v2_ids: [shounen.id] }
  let!(:anime_3) { create :anime, genre_v2_ids: [shounen.id] }
  let!(:anime_4) { create :anime }
  let!(:anime_5) { create :anime, genre_v2_ids: [shoujo.id] }
