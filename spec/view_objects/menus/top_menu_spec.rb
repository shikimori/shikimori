describe Menus::TopMenu do
  ...
  describe '#ranobe_seasons' do
    include_context :timecop, '2015-10-11'

    it do
      expect(view.ranobe_seasons.first).to be_kind_of Titles::StatusTitle
      expect(view.ranobe_seasons.second).to be_kind_of Titles::SeasonTitle
      expect(view.ranobe_seasons.map(&:text)).to eq %w(
        ongoing 2016 2015 2014 2013
      )
    end
  end
end
