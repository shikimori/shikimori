...
describe AnimeVideosQuery do
  let(:query) { AnimeVideosQuery.new adult, params }
  let(:params) { {} }
  ...
  context :adult do
    ...
    describe :fetch_ids do
      ...
      context :adult_not_filter do
        let!(:anime_video_censored) { create :anime_video, anime: create(:anime, censored: true) }
        let!(:anime_video_rating) { create :anime_video, anime: create(:anime, rating: Anime::ADULT_RATINGS.first) }

        it { should have(2).items }
      end

      context :common_filter do
        let!(:anime_video) { create :anime_video, anime: anime_1 }
        it { should be_blank }
      end
    end
  end
